...
use gobbler::{AUTH_SEED, states::{PoolState, AmmConfig, PoolStatusBitIndex}};
...
impl GobblerAmm {
    pub fn from_keyed_account(keyed_account: &KeyedAccount, amm_context: &AmmContext) -> Result<Self> {
        let data = &keyed_account.account.data[..]; // Skip 8-byte discriminator
        
        let pool_state = try_deserialize_unchecked_from_bytes_zc(data)?;
        Ok(Self {
    ...
    fn get_authority(&self) -> Pubkey {
        let (authority, __bump) = Pubkey::find_program_address(&[AUTH_SEED.as_bytes()], &gobbler::ID);
        authority
    }
}
...
impl SwapAccs {
    pub fn new(swap: &GobblerSwapAccs) -> Self {
        Self {
            swap_program: Pubkey::from_str("").unwrap(),
            payer: swap.payer,
...
impl Amm for GobblerAmm {
    ...
    fn program_id(&self) -> Pubkey {
        Pubkey::from_str("FUNSacu7QLHxBpfNWqn6NDa48Sk3449CLAkcinkkvfKs").unwrap()
    }
    ...
    fn update(&mut self, account_map: &AccountMap) -> Result<()> {
        ...
        let amm_config: AmmConfig = AmmConfig::try_from_slice(amm_config_data)?;
self.amm_config = Some(amm_config);     
    ...
    fn quote(&self, quote_params: &QuoteParams) -> Result<Quote> {
        ...
        let (total_fee, protocol_fee, creator_fee) = if zero_for_one {
            (
                amm_config.token_0_lp_rate,
                amm_config.protocol_fee,
                amm_config.creator_fee,
            )
        } else {
            (
        ...
        let swap_result = gobbler::curve::CurveCalculator::swap_base_input(
        ...
        let fee_amount = swap_result.total_fees;
    ...
    fn get_swap_and_account_metas(&self, swap_params: &SwapParams) -> Result<SwapAndAccountMetas> {
        ...
        let account_metas = SwapAccs::new(&GobblerSwapAccs {
            swap_program: Pubkey::from_str("FUNSacu7QLHxBpfNWqn6NDa48Sk3449CLAkcinkkvfKs").unwrap(),
